...
def convert_markdown_file(
    ...
    if use_versioning and VERSION_MANAGER_AVAILABLE:
        version_info = get_versioned_output_paths(input_path, OUTPUT_DIR, verbose)
        ...
        if output_path is None:
            output_path = os.path.join(OUTPUT_DIR, os.path.basename(os.path.splitext(input_path)[0] + '.docx'))
        
        if normalize and save_normalized:
            base_name = os.path.splitext(input_path)[0]
            normalized_output_path = os.path.join(OUTPUT_DIR, os.path.basename(f"{base_name}_normalized.md"))
    ...
    # 删除中间产物 normalized.md
    if normalized_output_path and os.path.exists(normalized_output_path):
        try:
            os.remove(normalized_output_path)
            if verbose:
                print(f"[Cleanup] Deleted normalized file: {normalized_output_path}")
        except Exception as e:
            if verbose:
                print(f"[Cleanup] Failed to delete normalized file: {e}")
