...
A deck with no `web` rows never loads `image-searcher.md`.
...
1. Extract all resource rows from the design spec and group them by `Acquire Via`; rows with `Status: Pending` or `Status: Failed` and `Acquire Via ∈ {web}` must all reach a terminal state before Executor starts
2. Generate prompts and/or run search (web rows) per [image-base.md](../references/image-base.md) §3 dispatch table
3. Verify every row reaches a terminal status: `Sourced` (web success), or `Needs-Manual`. `Failed` is not a terminal status: it means the current run did not generate that item, but the item remains retryable.
4. Re-derive image facts now that web files are in the folder — `python3 ${SKILL_DIR}/scripts/analyze_images.py <project_path>/images` — so `analysis/image_analysis.csv` reflects every acquired image before the Executor lays them out. Image facts are regenerated on use, never a stale store (see Step 4's image-facts note).
...
- [x] Each row: status is `Sourced` / `Needs-Manual` (no `Pending` or `Failed` remaining)
- [x] analyze_images.py re-run so image_analysis.csv covers the acquired web images
```
