...
Each `notes` value is **pure spoken narration**: write only what a presenter would say aloud. The note explains and connects; it must not just restate the words already on the slide.
...
| Plain prose only | No `#` heading line, no `- ` bullet lists, no `要点：① …` / `Key points:` lines, no `时长：2分钟` / `Duration:` annotations — embedded notes keep them verbatim and would be read aloud if exported |
| Number readability | Spell out figures when literal pronunciation is awkward (Chinese "百分之六十八" over "68%"; plain English integers and percentages are fine) |
