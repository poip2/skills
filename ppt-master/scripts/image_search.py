...
Searches openly-licensed image providers and downloads a
single best match.
...
# --- Inlined from the removed image_backends/backend_common.py ---
# download_image: download an image URL and save to disk.
import io
import time
...
EXT_TO_PIL_FORMAT = {".png": "PNG", ".jpg": "JPEG", ".jpeg": "JPEG", ".webp": "WEBP",
...
def detect_image_extension(image_bytes: bytes, content_type: str = None) -> str | None:
    ...
    if image_bytes.startswith(b"\xff\xd8\xff"): return ".jpg"
    ...
    if image_bytes.startswith((b"II*\x00", b"MM\x00*")): return ".tiff"
    return None

def _report_resolution(path: str) -> None:
    if HAS_PIL:
        try:
...
def download_image(url: str, path: str, headers: dict = None, timeout: int = 180) -> str:
    ...
    return save_image_bytes(response.content, path, content_type=response.headers.get("Content-Type"))
...
# rate), so the default concurrency is deliberately low — 3 keeps several
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.batch:
        ...
        try:
            ...
            return 130
        # A non-zero code flags rows that need manual attention.
        # It is a signal, not a halt — the workflow (image-base.md
