> See [`image-searcher.md`](./image-searcher.md) for path-specific behavior.
...
Active when at least one resource list row has `Acquire Via: web`. Rows with `user` / `formula` / `placeholder` are skipped.
...
|---|---|---|---|---|---|---|
| cover.png | 1280x720 | Cover background | Background | `user` | Existing | — |
...
**Required per non-skipped row**: `Acquire Via` and `Status`. `Reference` is required for every `web` row.
...
4. After the phase completes, summarize all `Needs-Manual` rows for the user — list filenames and where to place source files (`project/images/<filename>`).
...
Executor does NOT invoke `image_search.py` / `slice_images.py`.
...
- [x] {N} rows processed (`web`: {b})
- [x] {b} `Sourced`, {c} `Needs-Manual`
